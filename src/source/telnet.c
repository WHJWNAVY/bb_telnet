 ...
 * <andersen@codepoet.org> Modified 2001/05/07 to add ability to pass TTYPE to
 * remote host by Jim McQuillan <jam@ltsp.org> Modified 2004/02/11 to add
 * ability to pass the USER variable to remote host by Fernando Silveira
 * <swrh@gmx.net>
 */
...
// config:	default y
// config:	help
// config:	Telnet is an interface to the TELNET protocol, but is also
// commonly config:	used to test other simple protocols. config:
// config:config FEATURE_TELNET_TTYPE
// config:	bool "Pass TERM type to remote host"
// config:	default y
// config:	depends on TELNET
// config:	help
// config:	Setting this option will forward the TERM environment variable to
// the config:	remote host you are connecting to. This is useful to make sure
// that config:	things like ANSI colors and other control sequences behave.
// config:
...
// config:	depends on TELNET
// config:	help
// config:	Setting this option will forward the USER environment variable to
// the config:	remote host you are connecting to. This is useful when you need
// to config:	log into a machine without telling the username (autologin).
// This config:	option enables '-a' and '-l USER' options. config: config:config
// FEATURE_TELNET_WIDTH config:	bool "Enable window size autodetection" config:
// default y config:	depends on TELNET
...
static void record_signo(int signo);
...
int main(int argc, char **argv) {
    ...
    if (!*argv)
        bb_show_usage();
    host = *argv++;
    port = *argv ? bb_lookup_port(*argv++, "tcp", 23) : bb_lookup_std_port("telnet", "tcp", 23);
